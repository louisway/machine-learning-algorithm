...
def test_Mnist(data_num):
	...
	units_num=np.array([300,1])
	weight=train_nn(train_images,train_labels,units_num,1000,0.0005,0.0006)
	return weight

def valid_Mnist(data_num,weights):
	test_images,test_labels=ld.load_Binary_Mnist_Test_Data(data_num)
	test_labels=test_labels.reshape(test_labels.shape[0],1)
	test_images=test_images.astype(np.float32)/255
	test_images=concate_bias(test_images)
	inputs=[]
	inputs.append(test_images)
	for i in range(len(weights)-1):
		data_input=inputs[i]
		weight=weights[i]
		inter_output=np.dot(data_input,weight)
		...
		inputs.append(inter_output)
	net_output=np.dot(inputs[-1],weights[-1])
	h=logistic_function(net_output)
	h[h>0.5]=1
	h[h<=0.5]=0
...
if __name__=="__main__":
	weights=test_Mnist(60000)
	valid_Mnist(10000,weights)
	units_num=np.array([392,196,98,49,25,12,1])
	...
	#weights=train_nn(train_data,label_data,units_num,100,0.0001,0.05)
